...
for a = 1:numel(algoFolders)
    ...
    for i = 1:numel(probFolders)
        ...
        if ~isempty(d)
            ...
            % Plot 1st vs 2nd objective (Length vs Threat) or Length vs Altitude
            scatter(objs(:,1), objs(:,3), 50, 'filled', 'MarkerFaceColor', [0 0.4470 0.7410]);
            grid on;
            xlabel('Path Length (J1)');
            ylabel('Altitude (J3)');
            title(['Pareto Front (' algoName '): ' strrep(problemName, '_', ' ')]);
            set(fig1, 'Visible', 'on');
